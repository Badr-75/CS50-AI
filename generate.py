...
class CrosswordCreator():
    ...
    def select_unassigned_variable(self, assignment):
        ...
        mrv_vars = [v for v in unassigned if len(self.domains[v]) == mrv]

        if len(mrv_vars) == 1:
            return mrv_vars[0]

        # Degree heuristic (highest degree)
        max_degree = max(len(self.crossword.neighbors(v)) for v in mrv_vars)
        degree_vars = [v for v in mrv_vars if len(self.crossword.neighbors(v)) == max_degree]
